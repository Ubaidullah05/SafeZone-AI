...
// whenever it is reachable (see spec section 25).

export const RAW_VILLAGES = [
...
const W = { hazard: 0.30, slope: 0.20, popExp: 0.15, access: 0.15, facility: 0.10, historical: 0.10 }
...
export function applyAdjustments(villages, adj) {
...
export function runPipeline(villages, safeZones) {
  ...
  const remaining = Object.fromEntries(safeZones.map((z) => [z.id, z.capacity]))
  const allocated = Object.fromEntries(safeZones.map((z) => [z.id, 0]))
  ...
  const distribution = { SAFE: 0, MODERATE: 0, HIGH: 0, CRITICAL: 0 }
...
export function runFallbackRecommendation(villageId) {
  ...
  const scoredZones = RAW_SAFE_ZONES.map((z) => {
    ...
    const remaining = z.capacity // fallback mode scores against full capacity (no cross-village allocation)
...
export function runFallbackScenario(adjustments) {
