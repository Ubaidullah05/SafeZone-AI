...
import type { RecommendationResult, VillageResult } from '../types'

interface RelocationPanelProps {
  priorityList: VillageResult[]
  recommendation: RecommendationResult | null
  onSelectVillage: (id: string) => void
}

export default function RelocationPanel({ priorityList, recommendation, onSelectVillage }: RelocationPanelProps) {
  return (
      ...
      {recommendation && (
        <div className="rounded-2xl border border-golden/20 bg-golden/5 p-4">
          <p className="font-mono text-xs uppercase tracking-widest text-golden">
            Recommended Shelter — {recommendation.village_name}
          </p>
          ...
          {recommendation.recommended ? (
            <>
              ...
              <p className="mt-4 mb-2 text-sm font-medium text-surface-300">Why was this shelter recommended?</p>
            ...
            <p className="mt-2 text-sm text-surface-400">No suitable shelter could be found from current data.</p>
          )}
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-panel">
                <tr>
                  ...
                  <th className="px-3 py-3 font-mono font-normal text-surface-500">People to Move</th>
                  <th className="px-3 py-3 font-mono font-normal text-surface-500">Shelter</th>
